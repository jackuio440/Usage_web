(function () {
  ...
  async function load() {
    ...
    const rows = me.bookings.map((b) => {
      ...
      return `<tr><td>${state}</td><td>${esc(gpuLabel(b.gpus))}</td><td class="num">${fmtDateTime(s)}</td><td class="num">${fmtDateTime(e)}</td><td class="num">${hours(s, e).toFixed(1)}</td><td>${esc(b.purpose)}</td><td>${action}</td></tr>`;
    });
